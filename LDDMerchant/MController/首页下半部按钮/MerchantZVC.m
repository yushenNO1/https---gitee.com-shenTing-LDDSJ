...
- (void)viewDidLoad {
    ...
    redTf.placeholder = [NSString stringWithFormat:@"本次最多可转出%@", _fundAccount];
    redTf.tag = 104;
    ...
    [self.view addSubview:redTf];
     ...
     UILabel * labe1l1 = [[UILabel alloc] initWithFrame:CGRectMake(20 * kScreenWidth1, 134 + 10 * kScreenHeight1, 335 * kScreenWidth1, 30 * kScreenHeight1)];
     labe1l1.text = @"注: 转出到本商户个人版账户, 转出后个人版提现将有3%手续费, 慎用";
     labe1l1.textColor = [UIColor lightGrayColor];
     labe1l1.numberOfLines = 0;
     labe1l1.font = [UIFont systemFontOfSize:12];
     [self.view addSubview:labe1l1];
     
    UIButton * redBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    redBtn.frame = CGRectMake(175 / 2 * kScreenWidth1, 190 * kScreenHeight1, 200 * kScreenWidth1, 44 * kScreenHeight1);
    ...
    [redBtn setTitle:@"确认转出" forState:UIControlStateNormal];
...
- (void)handleAction1 {
    UITextField * redTf = [self.view viewWithTag:104];
     NSLog(@"asdasd-a-s-d----%@",redTf.text);
    float v = [redTf.text floatValue];
    if (v != 0) {
        DCPaymentView *payAlert = [[DCPaymentView alloc]init];
        payAlert.title = @"请输入支付密码";
        payAlert.detail = @"转出金额";
        payAlert.amount = [redTf.text floatValue];
        [payAlert show];
...
- (void) request {
    ...
    [manager.requestSerializer setValue:str forHTTPHeaderField:@"Authorization"];
    UITextField * redTf = [self.view viewWithTag:104];
    [manager POST:[NSString stringWithFormat:@"%@/api/v1/life/shop/transfer/toUser?payPassword=%@&amount=%@",LSKurl, _str, redTf.text] parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        }else if([responseObject[@"code"] integerValue] == 713){
             Alert_Show(@"支付密码错误");
            
        }else if([responseObject[@"code"] integerValue] == 708){
             Alert_Show(@"支付密码错误");
