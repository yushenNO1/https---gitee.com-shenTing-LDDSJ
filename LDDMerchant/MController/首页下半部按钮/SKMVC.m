...
-(void)configView
{
     ...
     bagImg.image = [UIImage imageNamed:@"WechatIMG5"];
     ...
     NSString *QRCode = [[NSUserDefaults standardUserDefaults]objectForKey:@"SKMVCQRCode"];
...
//          backLabel1.text = [NSString stringWithFormat:@"%@的店铺", _shopName];
...
- (void)request {
    ...
    [manager GET:[NSString stringWithFormat:@"%@/api/v1/pay/code/info/1",LSKurl] parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSLog(@"商家二维码---%@", responseObject);
        if ([responseObject[@"code"] intValue]== 0){
            //@"buess_ico_loading@3x"
             NSDictionary *myInfo = [[NSUserDefaults standardUserDefaults]objectForKey:@"infoMy"];
             
            self.QRImgView.image = [UIImage qrImageWithContent:[NSString stringWithFormat:@"LDDCode#%@",responseObject[@"data"][@"payCode"]] logo:[UIImage imageNamed:@"组-1_90"] size:200 red:38 green:38 blue:38];
             [[NSUserDefaults standardUserDefaults]setObject:[NSString stringWithFormat:@"%@&Code#%@",[NSString stringWithFormat:@"%@/index.php?m=Mobile&c=User&a=reg&spreader=%@",LSKurl1, myInfo[@"userVo"][@"spreadCode"]], responseObject[@"data"]] forKey:@"SKMVCQRCode"];
//             [self loadMyInfo];
             NSDictionary *shopInfo = [[NSUserDefaults standardUserDefaults]objectForKey:@"MyInfo"];
