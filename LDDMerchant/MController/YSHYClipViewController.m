...
#define kBI_LI_Height              kScreenHeight / 667
#define kBI_LI_Width              kScreenWidth / 375
...
-(instancetype)initWithImage:(UIImage *)image
{
    if(self = [super init])
    {
        ...
        self.radius = 335 / 2 * kBI_LI_Width;
        self.scaleRation =  3;
    }
    return  self;
}
...
//    self.navigationController.navigationBar.tintColor = [UIColor backGray];
//    UIBarButtonItem * rightItem = [[UIBarButtonItem alloc]initWithTitle:@"使用" style:(UIBarButtonItemStylePlain) target:self action:@selector(clipBtnSelected:)];
//    self.navigationItem.rightBarButtonItem = rightItem;
}

- (void) layout {
    [self.navigationController.navigationController.navigationBar setTitleTextAttributes:@{NSForegroundColorAttributeName:[UIColor whiteColor],NSFontAttributeName:[UIFont systemFontOfSize:17]}];
}
...
-(void)CreatUI
{
    ...
    [clipBtn setFrame:CGRectMake(20*kBI_LI_Width, self.view.frame.size.height - 80*kBI_LI_Height, 160*kBI_LI_Width, 60*kBI_LI_Height)];
    [self.view addSubview:clipBtn];
    
    UIButton * alipBtn = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [alipBtn setTitle:@"选定" forState:UIControlStateNormal];
    [alipBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [alipBtn addTarget:self action:@selector(alipBtnSelected:) forControlEvents:UIControlEventTouchUpInside];
    [alipBtn setBackgroundColor:[UIColor grayColor]];
    [alipBtn setAlpha:1];
    alipBtn.layer.cornerRadius = 5;
    alipBtn.layer.masksToBounds = YES;
    [alipBtn setFrame:CGRectMake(195*kBI_LI_Width, self.view.frame.size.height - 80*kBI_LI_Height, 160*kBI_LI_Width, 60*kBI_LI_Height)];
    [self.view addSubview:alipBtn];
...
-(void)alipBtnSelected:(UIButton *)btn
{
